...
__global__ void bounding_boxes( int No_of_C180s, 
               ...
               float *d_bounding_xyz, float *CMx, float *CMy,float *CMz)
{
...
if ( rank < No_of_C180s )
    ...
    while ( atom + 32 < 180 )
        {
        atom += 32;
        if ( minx[tid] > d_XP[rank*192+atom] ) minx[tid] = d_XP[rank*192+atom];
        if ( maxx[tid] < d_XP[rank*192+atom] ) maxx[tid] = d_XP[rank*192+atom];
        if ( miny[tid] > d_YP[rank*192+atom] ) miny[tid] = d_YP[rank*192+atom];
        if ( maxy[tid] < d_YP[rank*192+atom] ) maxy[tid] = d_YP[rank*192+atom];
        if ( minz[tid] > d_ZP[rank*192+atom] ) minz[tid] = d_ZP[rank*192+atom];
        if ( maxz[tid] < d_ZP[rank*192+atom] ) maxz[tid] = d_ZP[rank*192+atom];
        sumx[tid] += d_XP[rank*192+atom];
        sumy[tid] += d_YP[rank*192+atom];
        sumz[tid] += d_ZP[rank*192+atom];
                                               // move present value to past value
