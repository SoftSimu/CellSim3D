...
//  printf("(%d, %d, %d) %d %d\n", blockIdx.x, blockDim.x, threadIdx.x, fullerene, No_of_C180s);
	...
	if ( fullerene < No_of_C180s )
	{
	  	...
	  	int posx = (int)(( CMx[fullerene] - floor( CMx[fullerene] / boxMax.x) * boxMax.x )/DLp.x); 	
 	  	int posy = (int)(( CMy[fullerene] - floor( CMy[fullerene] / boxMax.y) * boxMax.y )/DLp.y); 	
	  	int posz = (int)(( CMz[fullerene] - floor( CMz[fullerene] / boxMax.z) * boxMax.z )/DLp.z); 		
...
__global__ void Ghost_Cells_Pack_PBC_X(int No_of_Ghost_cells_buffer, int No_of_Ghost_cells_buffer_R, int* d_Ghost_Cells_ind, float3 boxMax,  float R_ghost_buffer,
	...
	if( cell < No_of_Ghost_cells_buffer ) {
		...
		if( cell < No_of_Ghost_cells_buffer_R ){
		
		
			if( d_CMx[ghost_cell] > boxMax.x - R_ghost_buffer  - 0.2)
				d_X_gc_buffer[cell*192 + tid] = d_X[192*ghost_cell + tid] - boxMax.x;
			else 
				d_X_gc_buffer[cell*192 + tid] = d_X[192*ghost_cell + tid];
			...
			if(tid == 0){
				...
				if( d_CMx[ghost_cell] > boxMax.x - R_ghost_buffer - 0.2)
					d_CMx_gc_buffer[cell] = d_CMx[ghost_cell] - boxMax.x;
				else 
					d_CMx_gc_buffer[cell] = d_CMx[ghost_cell];
			...
			if( d_CMx[ghost_cell] < R_ghost_buffer + 0.2)
				d_X_gc_buffer[cell*192 + tid] = d_X[192*ghost_cell + tid] + boxMax.x;
			else 
				d_X_gc_buffer[cell*192 + tid] = d_X[192*ghost_cell + tid];
...
__global__ void Ghost_Cells_Pack_PBC_Y(int No_of_Ghost_cells_buffer, int No_of_Ghost_cells_buffer_R, int* d_Ghost_Cells_ind, float3 boxMax,  float R_ghost_buffer,
	...
	if( cell < No_of_Ghost_cells_buffer ) {
		...
		if( cell < No_of_Ghost_cells_buffer_R ){
			...
			if( d_CMy[ghost_cell] > boxMax.y - R_ghost_buffer - 0.2)
				d_Y_gc_buffer[cell*192 + tid] = d_Y[192*ghost_cell + tid] - boxMax.y;
			else 	
				d_Y_gc_buffer[cell*192 + tid] = d_Y[192*ghost_cell + tid];
			...
			if(tid == 0){
				...
				if( d_CMy[ghost_cell] > boxMax.y - R_ghost_buffer - 0.2 )
					d_CMy_gc_buffer[cell] = d_CMy[ghost_cell] - boxMax.y;
				else 	
					d_CMy_gc_buffer[cell] = d_CMy[ghost_cell];
			...
			if( d_CMy[ghost_cell] < R_ghost_buffer + 0.2 )
				d_Y_gc_buffer[cell*192 + tid] = d_Y[192*ghost_cell + tid] + boxMax.y;
			else 	
				d_Y_gc_buffer[cell*192 + tid] = d_Y[192*ghost_cell + tid];
...
__global__ void Ghost_Cells_Pack_PBC_Z(int No_of_Ghost_cells_buffer, int No_of_Ghost_cells_buffer_R, int* d_Ghost_Cells_ind, float3 boxMax, float R_ghost_buffer,
	...
	if( cell < No_of_Ghost_cells_buffer ) {
		...
		if( cell < No_of_Ghost_cells_buffer_R ){
			...
			if( d_CMz[ghost_cell] > boxMax.z - R_ghost_buffer - 0.2 )
				d_Z_gc_buffer[cell*192 + tid] = d_Z[192*ghost_cell + tid] - boxMax.z;
			else
				d_Z_gc_buffer[cell*192 + tid] = d_Z[192*ghost_cell + tid];
			...
			if(tid == 0){
				...
				if( d_CMz[ghost_cell] > boxMax.z - R_ghost_buffer - 0.2 )
					d_CMz_gc_buffer[cell] = d_CMz[ghost_cell] - boxMax.z;
				else 
					d_CMz_gc_buffer[cell] = d_CMz[ghost_cell];
			...
			if( d_CMz[ghost_cell] < R_ghost_buffer + 0.2 )
				d_Z_gc_buffer[cell*192 + tid] = d_Z[192*ghost_cell + tid] + boxMax.z;
			else
				d_Z_gc_buffer[cell*192 + tid] = d_Z[192*ghost_cell + tid];
