	...
	volumes<<<No_of_C180s,192>>>(No_of_C180s, d_C180_56,
  	...
  	CalculateConForce<<<No_of_C180s,threadsperblock>>>( No_of_C180s, d_C180_nn, d_C180_sign,
															...
															d_CMxPin, d_CMyPin, d_CMzPin,
                                                     		...
                                                     		d_viscotic_damp,
                                                     		Xdiv, Ydiv, Zdiv, boxMax, Moving_Min, Moving_Max,
															...
															MaxNeighList_ecm, wall_adhesion, LJ_epsilon, LJ_sigma, Single_wall,
...
//   if (compress) StepCompression = int (0.2/Compression_step_size);
  ...
  for ( step = 1; step < Time_steps+1 + equiStepCount; step++)
  {
	...
	if (compress && !Single_wall){
		if ( (Sphere_radius*2 * (1 - compression_ratio)) < (Moving_Max.z - Moving_Min.z) ){
			// boxMax.z -= Compression_step_size;
			// BoxMin.z += Compression_step_size;
			Moving_Max.z -= Compression_step_size;
			Moving_Min.z += Compression_step_size;
			// printf("Box max %f and Box min %f ", Moving_Max.z , Moving_Min.z);
		} else {
			compress = false;
			StepCompression = 100000000;
			if (rank == 0) printf("   Compression finished. height of box:  %f\n" , Moving_Max.z - Moving_Min.z);
		}

	} else if (compress){ //Set the middle by hand -> 25
		if ( (Sphere_radius* (1 - compression_ratio)) < (Initial_Center - Moving_Min.z) ){
			// BoxMin.z += Compression_step_size;
			Moving_Min.z += Compression_step_size;
			// printf("Box max %f and Box min %f ", Moving_Max.z , Moving_Min.z);
		} else {
...
int read_json_params(const char* inpFile){
    ...
    if (boxParams == Json::nullValue){
		...
		Moving_Max.x = boxMax.x;
		Moving_Max.y = boxMax.y;
		Moving_Max.z = boxMax.z;
		Moving_Min.x = BoxMin.x;
		Moving_Min.y = BoxMin.y;
		Moving_Min.z = BoxMin.z;
